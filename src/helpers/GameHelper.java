...
public class GameHelper {
	...
	/**
	 * Computes the resize factor keeping the aspect ratio of the size, to fit
	 * the canvas.
	 *
	 * @param width
	 ...
	 * @param canvas
	 *            the canvas in which inscribe the image
	 * @return the scale factor
	 */
	public static float scaleAspectRatio(final float width, final float height,
			final float canvas) {
		final float longest = Math.max(width, height);
		final float scale = canvas / longest;
		return scale;
	}
